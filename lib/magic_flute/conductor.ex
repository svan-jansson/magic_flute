defmodule MagicFlute.Conductor do
  ...
  def start_performance(players, bpm, signature, length_in_bars) do
    %{active: active} = DynamicSupervisor.count_children(__MODULE__)
    start(players, bpm, signature, length_in_bars, active > 0)
  end

  defp start(players, bpm, signature, length_in_bars, _running = false) do
  ...
  defp start(_players, _bpm, _signature, _length_in_bars, _running = true) do
